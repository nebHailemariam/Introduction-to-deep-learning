...
class GELU:
    ...
    def backward(self, dLdA):
        ...
        dAdZ = 0.5*(1 + scipy.special.erf(self.Z/(2**0.5))) + \
            self.Z/((2*np.pi)**0.5)* np.exp(-(self.Z * self.Z)/2)
        dLdZ = dLdA * dAdZ
...
class Softmax:
    ...
    def backward(self, dLdA):
        ...
        for i in range(N):
            ...
            for m in range(C):
                for n in range(C):
                    J[m,n] = self.A[i][m]*(1 - self.A[i][m]) if n == m else -self.A[i][m]*self.A[i][n]# TODO
