# DO NOT import any additional 3rd party external libraries as they will not
# be available to AutoLab and are not needed (or allowed)

from flatten import *
from Conv1d import *
from linear import *
from activation import *
from loss import *
import numpy as np
import os
import sys

sys.path.append('mytorch')


class CNN_SimpleScanningMLP():
    def __init__(self):
        # Your code goes here -->
        # self.conv1 = ???
        # self.conv2 = ???
        # self.conv3 = ???
        # ...
        # <---------------------
        # Define the convolutional layers
        self.conv1 = Conv1d(in_channels=24, out_channels=8, kernel_size=8, stride=4)
        self.conv2 = Conv1d(in_channels=8, out_channels=16, kernel_size=1, stride=1)
        self.conv3 = Conv1d(in_channels=16, out_channels=4, kernel_size=1, stride=1)
        
        # Define layers in a sequence
        self.layers = [
            self.conv1,
            ReLU(),
            self.conv2,
            ReLU(),
            self.conv3,
            Flatten()
        ]

    def init_weights(self, weights):
        # Load the weights for your CNN from the MLP Weights given
        # w1, w2, w3 contain the weights for the three layers of the MLP
        # Load them appropriately into the CNN - 
        #    1. For each conv layer, have a look at the shape of its weight matrix
        #    2. Look at the shapes of w1, w2 and w3
        #    3. Figure out appropriate reshape and transpose operations

        w1, w2, w3 = weights
        self.conv1.conv1d_stride1.W = np.transpose(np.reshape((w1.T),(8, 8, -1)),(0, 2, 1))
        self.conv2.conv1d_stride1.W = np.transpose(np.reshape((w2.T),(16, 1, -1)),(0, 2, 1))
        self.conv3.conv1d_stride1.W = np.transpose(np.reshape((w3.T),(4, 1, -1)),(0, 2, 1))

    def forward(self, A):
        """
        Do not modify this method

        Argument:
            A (np.array): (batch size, in channel, in width)
        Return:
            Z (np.array): (batch size, out channel * out width)
        """

        Z = A
        for layer in self.layers:
            Z = layer.forward(Z)
        return Z

    def backward(self, dLdZ):
        """
        Do not modify this method

        Argument:
            dLdZ (np.array): (batch size, out channel, out width)
        Return:
            dLdA (np.array): (batch size, in channel, in width)
        """
        dLdA = dLdZ
        for layer in self.layers[::-1]:
            dLdA = layer.backward(dLdA)
        return dLdA


class CNN_DistributedScanningMLP():
    def __init__(self):
        # Your code goes here -->
        # self.conv1 = ???
        # self.conv2 = ???
        # self.conv3 = ???
        # ...
        # <---------------------
        self.conv1 = Conv1d(24, 2, kernel_size=2, stride=2)
        self.conv2 = Conv1d(2, 8, kernel_size=2, stride=2)
        self.conv3 = Conv1d(8, 4, kernel_size=2, stride=1)
        self.layers = [self.conv1, ReLU(), self.conv2, ReLU(), self.conv3, Flatten()]

    def __call__(self, A):
        # Do not modify this method
        return self.forward(A)

    def init_weights(self, weights):
        # Load the weights for your CNN from the MLP Weights given
        # w1, w2, w3 contain the weights for the three layers of the MLP
        # Load them appropriately into the CNN

        w1, w2, w3 = weights
        # Reshape and transpose the weights for the first convolutional layer (conv1)
        self.conv1.conv1d_stride1.W = np.transpose(np.reshape(w1[:, :2].T, (2, 8, -1))[:, :2, :], (0, 2, 1))

        # Reshape and transpose the weights for the second convolutional layer (conv2)
        self.conv2.conv1d_stride1.W = np.transpose(np.reshape(w2[:, :8].T, (8, 4, -1))[:, :2, :], (0, 2, 1))

        # Reshape and transpose the weights for the third convolutional layer (conv3)
        self.conv3.conv1d_stride1.W = np.transpose(np.reshape(w3.T, (4, 2, -1)), (0, 2, 1))
        
    def forward(self, A):
        """
        Do not modify this method

        Argument:
            A (np.array): (batch size, in channel, in width)
        Return:
            Z (np.array): (batch size, out channel , out width)
        """

        Z = A
        for layer in self.layers:
            Z = layer.forward(Z)
        return Z

    def backward(self, dLdZ):
        """
        Do not modify this method

        Argument:
            dLdZ (np.array): (batch size, out channel, out width)
        Return:
            dLdA (np.array): (batch size, in channel, in width)
        """
        dLdA = dLdZ
        for layer in self.layers[::-1]:
            dLdA = layer.backward(dLdA)
        return dLdA
