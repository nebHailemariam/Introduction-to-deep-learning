...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        if eval == False:
            ...
            self.running_M = (
                self.alpha * self.running_M + (1 - self.alpha) * self.NZ
            )  # TODO
            self.running_V = (
                self.alpha * self.running_V + (1 - self.alpha) * self.BZ
            )  # TODO
        else:
            # inference mode
            self.NZ = (self.Z - self.running_M) / (
                self.running_V + self.eps
            ) ** 0.5  # TODO
            self.BZ = self.BW * self.NZ + self.Bb  # TODO
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -0.5 * np.sum(
            (dLdNZ * (self.Z - self.M) * ((self.V + self.eps) ** (-3 / 2))), axis=0
        )  # TODO
