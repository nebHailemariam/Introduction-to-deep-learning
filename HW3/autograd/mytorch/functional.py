...
def conv1d_stride1_backward(dLdZ, A, weight, bias):
    ...
    for w in range(w_in):
        axs = ([1, 2], [0, 2])
        dLdA[:, :, w] = np.tensordot(
            pdLdZ[:, :, w:w+kernel_size], flipW, axes=axs)
...
def conv2d_stride1_backward(dLdZ, A, weight, bias):
    ...
    for kh in range(kernel_size):
        for kw in range(kernel_size):
            axs = ([0, 2, 3], [0, 2, 3])
            dLdW[:, :, kh, kw] += np.tensordot(dLdZ,
                                               A[:, :, kh:kh+h_out, kw:kw + w_out], axes=axs)
...
def ctc_loss_backward(grad, logits, input_lengths, gammas, extended_symbols):
    ...
    for batch_itr in range(B):
        ...
        for r in range(gamma.shape[1]):
            dY[:input_lengths[batch_itr], batch_itr, ext_symbols[r]] -= grad * gamma[:, r] / logits_t[:,ext_symbols[r]]
